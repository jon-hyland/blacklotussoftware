 ...
 * All external functions (c) 1994-1998 Black Lotus Software.       *
 * This program will create and modify HSFILE.DAT for high score    *
 * list.                                                            *
 *                                                                  *
 * Written and compiled using Borland C++ v5.02.                    *
 *                                                                  *
...
/********************************************************************
 * Other Notes :                                                    *
 * 	Even though it's usually bad programming practice to be so    *
 *	  	dependant on global variables, the nature of this program     *
 *		requires easy access to it's many data items.  None of the    *
 *		functions in this file would have any use outside of this     *
 *		game.                                                         *
 ********************************************************************/
...
char ver_num[] = "v2.1SE";                // version number
...
void flip_block(Piece_struct &);
void draw_block(int, int, Piece_struct &);
void erase_block(int, int, Piece_struct &);
void draw_area(Area_struct &);
void draw_next(Piece_struct &);
void new_block(void);
      ...
      do								// loop until Esc or dead, come here evey move
      {
         ...
         if (!dead)						// if player is still alive...
         {
            ...
            if (kbhit())				// if key has been pressed
            {
               ...
               switch (key)			// check key for following options
               {
               	case '\0' : key = getch(); // if key is extended char,
                  				...
                  				switch (key)	// check trailing char for options
                              {
                              	case 'H' :  // up arrow (flip)
                                 				erase_block(bx, by, piece);
                                 			  	flip_block(piece);
                                            	if (bad_move())
                                             	for(j=1; j<=3; j++)
                                                	flip_block(piece);
                                             draw_block(bx, by, piece);
                                             break;
                  ...
                  case 'R' :  reset_high_score();					// [R]eset
                              draw_window(30,10,47,13,4,"Info"); // High Scores
                              textcolor(YELLOW);
                              gotoxy(34,11);
                              cprintf("High Scores");
                              gotoxy(36,12);
                  ...
                  case 'E' :	if (sound_on)							// Sound [E]ff
                              ...
                              break;
               	*/
                  case 'V' :	view_help_doc();						// [V]iew Help
                  				break;
               }
            }
            else 											// if keyboard not hit
            {
               ...
               {
               	erase_block(bx, by, piece);   // block fall one line
                  by++;
                  draw_block(bx, by, piece);
               }
            }
...
void flip_block(Piece_struct &piece)
...
void draw_block(int x, int y, Piece_struct &piece)
...
void erase_block(int x, int y, Piece_struct &piece)
...
void draw_area(Area_struct &scr)
...
void draw_next(Piece_struct &next)
...
// piece.  If first block in the game, current piece is random.
{
   ...
   {
   	next = proto[random(7)];      // next is random prototype
      first = 0;							// first piece is false
   }
   ...
   by = 0;                          // place at top
   draw_block(bx, by, piece);			// draw it (here we go, baby)
...
// total solid lines, game level, and if next piece showing (half off).
{
   ...
   if (times)									// if there was solid line(s)
   {
      ...
      nosound();
      */
      draw_area(scr);							// draw new playing area
   }
}


void view_help_doc(void)
// This function will try to open the help document, if it didn't work
// a message will be displayed.  Otherwise, the screen fades and
// view_text() is called to view text file.  User can scroll through
// text with [arrows] or [pgup]/[pgdwn] and must exit with [Esc].
{
	fstream exist;                         // temporary file variable

   exist.open("BLOCK.TXT", ios::in);      // try to open temorary
   if (!exist)                            // if it can't,
   {
      exist.close();
		draw_window(30,10,47,14,4,"Info");	// draw failure window..
		textcolor(YELLOW);
      gotoxy(34, 11);
		cprintf("Way to go");
      gotoxy(32, 12);
      ...
      cprintf("deleted it.");
		cursor();
		pause();                            // wait for keypress
      erase_window(30,10,47,14);				// erase window
   }
   else												// if it can open,
   {
   	exist.close();								// close temporary
      fadeout();                          // fade screen away
      view_text("BLOCK.TXT");             // view text file
      fadeout();                          // fade screen again
      draw_screen();                      // redraw main program screen
		if (next_piece)
      	draw_next(next);
      textcolor(YELLOW);						// redraw points / level..
   	textbackground(BLUE);
   	gotoxy(20, 21);
   	cprintf("%d", points);
   	gotoxy(20, 19);
	...
	draw_area(scr);								// redraw playing area
   draw_block(bx, by, piece);             // redraw current block
}
...
void write_high_score(void)
{
   ...
   write_file = fopen("HSFILE.DAT", "wb");
	for(int i=0; i<10; i++)
	   fwrite(&hs_list[i], sizeof(hs_list[i]), 1, write_file);
   fclose(write_file);
}
...
void read_high_score(void)
{
   ...
   read_file = fopen("HSFILE.DAT", "rb");
	for(int i=0; i<10; i++)
   	fwrite(&hs_list[i], sizeof(hs_list[i]), 1, read_file);
   fclose(read_file);
}
...
// window.
{
   ...
   draw_area(scr);                     // redraw affected screen areas...
   draw_block(bx, by, piece);
   textbackground(BLUE);
