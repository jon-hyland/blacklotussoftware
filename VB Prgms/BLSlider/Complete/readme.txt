...
After installing, before you do anything else, run Reset.exe located in the current folder.  This will reset the initialization file so that the program will know that folder to start out in.



1. INTRODUCTION

Black Lotus Slider is a program written to view all the picture images in a specified folder.  You point the program towards a folder, specify the filespec or wildcard to be viewed, and let the slide show start.  There are quite a few options - a randomize feature, time delay in seconds that can be toggled on or off, optional viewing of Prev / Next buttons in view mode just to start.  You also have the option to change the full-sized picture window to any corner or center at half the size.  This becomes an incredibly useful and fun utility if you happen to own a digital camera and want to view your pictures in a fun, easy-to-use format.



2. INSTALLATION INSTRUCTIONS

Copy or extract all the included files to a new folder. Ex: "C:\BLSLIDER\".  Open the folder you just created and be sure to double-click Reset.exe first.  Now you can run BLSlider.exe to start the Black Lotus Slider.
...
A.  Black Lotus Slider will correctly recognize .BMP, .JPG, .GIF, .ICO, .WMF and .EMF file formats with or without the proper extension.
...
If your BLSINI.DAT file becomes corrupt while still in the program, clicking on Reset Options will return both the program and the datafile to the default settings.  The default viewing folder is the folder the program is running from, and the default filespec is (*.JPG).

If your BLSINI.DAT file becomes corrupt and the program won't fully load you will have to run Reset.exe.  This will also reset the data file to its correct settings.
...
LINE 7: Screen mode. Integer from 1 to 6.  6 is fullscreen.
	...
	respectively: top-left, top-right, bottom-left,
	bottom-right, center.
